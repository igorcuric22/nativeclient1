...
const Dim11 = () => {
    ...
    if (canvas) {
     ...
     drawCoordinateSystem(ctx, canvas.width, canvas.height);
      ...
      const point1 = {x:50,y:0};
        rot(point1,i); // Rotates the point by 45 degrees
        const point2 = {x:100,y:0};
        rot(point2,i); // Rotates the point by 45 degrees
        const point3 = {x:point1.x,y:point1.y};
        rot(point3,i); // Rotates the point by 45 degrees
  ...
  const rot = (point, i) => {
    ...
    point.x = x * cosI + y * sinI;
    point.y = -x * sinI + y * cosI;
};
