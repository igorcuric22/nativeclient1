import React, { useEffect, useRef } from 'react';
import { View } from 'react-native';
import Canvas from 'react-native-canvas';

const Dim12 = () => {
  const requestRef = useRef(); // Store the animation frame ID
  const angleRef = useRef(0); // Track the angle for rotation

  const handleCanvas = (canvas) => {
    if (canvas) {
      canvas.width = 400;
      canvas.height = 400;
      const ctx = canvas.getContext('2d');
      drawCoordinateSystem(ctx, canvas.width, canvas.height);

      // Start the animation loop
      const animate = () => {
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        drawCoordinateSystem(ctx, canvas.width, canvas.height); // Redraw coordinate system

        angleRef.current += 0.01; // Increment angle
        const angle = angleRef.current;

        const point1 = { x: 50, y: 0 };
        rot(point1, angle);
        const point2 = { x: 100, y: 0 };
        const point3 = { x: point1.x, y:point1.y };

        
        rot(point2, angle);
        rot(point3, angle);

        drawx(ctx, canvas.width, canvas.height, point2.x, point2.y, point3.x, point3.y);

        if (angle <= 2 * Math.PI) {
          requestRef.current = requestAnimationFrame(animate); // Continue the animation
        }
      };

      // Start the animation
      requestRef.current = requestAnimationFrame(animate);
    }
  };

  // Cancel animation on component unmount
  useEffect(() => {
    return () => cancelAnimationFrame(requestRef.current);
  }, []);

  const drawx = (ctx, width, height, x22, y22, x33, y33) => {
    const x0 = width / 2;
    const y0 = height / 2;

    // Draw the first circle
    ctx.beginPath();
    ctx.arc(x0, y0, 50, 0, Math.PI * 2);
    ctx.strokeStyle = 'blue';
    ctx.lineWidth = 1;
    ctx.stroke();

    // Draw the second circle
    ctx.beginPath();
    ctx.arc(x0 + x22, y0 + y22, 50, 0, Math.PI * 2);
    ctx.stroke();

    // Draw the line
    ctx.beginPath();
    ctx.moveTo(x0, y0);
    ctx.lineTo(x0 + x22, y0 + y22);
    ctx.lineTo(x0 + x22 + x33, y0 + y22 + y33);
    ctx.strokeStyle = 'green';
    ctx.lineWidth = 1;
    ctx.stroke();
  };

  const drawCoordinateSystem = (ctx, width, height) => {
    const centerX = width / 2;
    const centerY = height / 2;

    // Draw vertical line (Y-axis)
    ctx.beginPath();
    ctx.moveTo(centerX, 0);
    ctx.lineTo(centerX, height);
    ctx.strokeStyle = 'red';
    ctx.lineWidth = 1;
    ctx.stroke();

    // Draw horizontal line (X-axis)
    ctx.beginPath();
    ctx.moveTo(0, centerY);
    ctx.lineTo(width, centerY);
    ctx.stroke();
  };

  const rot = (point, angle) => {
    const { x, y } = point;
    const cosA = Math.cos(angle);
    const sinA = Math.sin(angle);

    point.x = x * cosA + y * sinA;
    point.y = -x * sinA + y * cosA;
  };

  return (
    <View style={{ alignItems: 'center', justifyContent: 'center', flex: 1 }}>
      <Canvas ref={handleCanvas} style={{ borderWidth: 1, borderColor: 'black' }} />
    </View>
  );
};

export default Dim12;

